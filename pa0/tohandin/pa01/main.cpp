			...
			for(int x = 0;x != imageWidth; x ++){
				for(int c = 0;c != chanls ; c++)
				{
					if((c+chanls)%chanls==i)					
						...
						outputfile[c + x* chanls+ y * imageWidth* chanls]=data[y][x][i];
				}
			}
		}
	}
	...
	switch (key){
						 ...
						 for(int x = 0;x != imageWidth; x ++){
							 for(int c = 0;c != chanls ; c++)
							 {
								 if((c+chanls)%chanls==i)					
									 ...
									 display_data[c + x* chanls+ y * imageWidth* chanls]=data[imageHeight-1-y][x][i];
							 }
						 }
					 }
		...
		case 'w':
				 if (do_output==1){
					 ...
					 std::cout << "output filename is not spcefiied " << std::endl;
				 }
				 dis();
				 break;
		case 'q':
